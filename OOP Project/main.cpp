...
class Maze {
    ...
    static const int grid_w = 48;  // 1920/40 = 48 columns
    static const int grid_h = 27;  // 1080/40 = 27 rows
    int walls[grid_w][grid_h] = { 0 };  // Initialize all to 0
...
public:
    Maze() {
        ...
        if (!backgroundTexture.loadFromFile("C:/C++/2nd Semester/OOP Project/OOP Project/background.jpg")) {
            cout << "Failed to load background texture!" << endl;
        }
        else {
            backgroundTexture.setRepeated(true);  // Allow repetition
            backgroundSprite.setTexture(backgroundTexture);
            backgroundSprite.setTextureRect(IntRect(0, 0, 1920, 1080)); // Full window size
        }
...
public:
    ...
    player() {
        ...
        // 3) In your player constructor, set the texture on the circle:
        pacman.setRadius(15.f);  // Original was 40.f - now matches classic size
    ...
    void update(Maze& maze) {
        if (blockedByWall) return;// Player is blocked, wait for input
        ...
        // Check wall collision using bounds (scan full maze walls)
    ...
    void resetPosition() {
        ...
        direction = Vector2f(0, 0);
        blockedByWall = false;
        // Reset movement's last direction
...
void movement::input(player& p, Maze& maze) {
    ...
    if (lastValidDir.x != 0.f || lastValidDir.y != 0.f) {
        ...
        // Trim the bounding box to fit inside sprite visually
        futureBounds.left += 2.f;
        futureBounds.top += 2.f;
        futureBounds.width -= 4.f;
        futureBounds.height -= 4.f;
...
public: CircleShape shape;
      ...
      int stuckFrames = 0;
      const int maxStuckFrames = 10; // ~1 second at 30fps
...
public:
    ...
    void update(Maze& maze, const player& p) {
        // Handle scatter/chase mode timing
        ...
        // Choose best direction from available options
        ...
        // If completely stuck, allow reversing direction
    ...
    void makeVulnerable() {
        vulnerable = true;
        powerClock.restart();
        shape.setFillColor(Color::Blue);  // Indicate vulnerability
    }
...
public:
    ...
    Menu() {
        selected_option = 0;
        currentState = menu;
        if (!menuMusic.openFromFile("C:/C++/2nd Semester/OOP Project/OOP Project/645631_Lunar-Abyss.ogg")) {
            cout << "Error: Could not load menu background music!" << endl;
        }
        menuMusic.setLoop(true);  // Enable looping
        menuMusic.play();         // Start playing the music

        // Attempt to load background texture
        if (!backgroundTexture.loadFromFile("C:/C++/2nd Semester/OOP Project/OOP Project/bg.jpg")) {
            cout << "Error: Could not load menu background texture!" << endl;
        ...
        // Scale background to fit full screen
        Vector2u windowSize = { 1920, 1080 }; // or window.getSize() if accessible
...
public:
    ...
    int lives = 3;
    bool gameOver = false;
    float ghostReleaseDelay = 4.0f; // 4 seconds delay
    Ghost* ghostQueue[3];  // Fixed-size array for ghost queue
        ...
        if (gameMenu.getState() == menu) {
            ...
            if (gameMenu.getState() == menu) {
                if (gameMenu.menuMusic.getStatus() != SoundSource::Playing) {
                    gameMenu.menuMusic.play();  // Start the music if not already playing
                }
                gameMenu.render(wind);
            }
            else {
                gameMenu.menuMusic.stop();  // Stop the music when leaving the menu
            }
            ...
            RectangleShape scoreBackground(Vector2f(500, 50));
            scoreBackground.setPosition(40.f, 0.f);
    ...
    void update() {
        ...
        if (gameMenu.getState() == play) {
            ...
            // Update only the released ghosts
            ...
            // Expand player's bounds slightly to ensure food near walls is detected
